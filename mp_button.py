...
class Button(object):
  ...
  def __init__(self, pin, rest_state = False, callback = None, internal_pullup = False, internal_pulldown = False, debounce_time = None, dataBuffer = None):
    self.pin_number = pin
    ...
    if self.current_debounced_state != self.previous_debounced_state:
      if self.current_debounced_state != self.rest_state:
        ...
        if self.callback != None:
          self.callback(self.pin_number, Button.PRESSED, self.dataBuffer)
      else:
        self.active = False
        if self.callback != None:
          self.callback(self.pin_number, Button.RELEASED, self.dataBuffer)
